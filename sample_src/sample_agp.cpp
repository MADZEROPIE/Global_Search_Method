...
int main(int argc, char* argv[]) {
    ...
    vector <int> r_vec = { 2,3,4,5 };
    
    for(int r = 2; r < 6; ++r) {  // Casual(?)
    ...
    int n = 30; // Number of problems
    int m = 4; //Number of constraints
    vector<std::string> names_vecC = { "SheckConstr", "HillConstr" };
    std::cout << "Генерируется семейство функций...\n";
    auto type = SheckelOnly; // USE SheckelOnly or HillOnly. Others will work as HillOnly.
